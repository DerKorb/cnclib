function cnclib(userOptions)
{
	...
	this.options = { z_safe: 2, nPathTrials: 10000, drillClockwise: true, maxVSpeed: 100, maxHSpeed: 400, stepFactor: 0.75 };
	var parts = [];
	var that = this;
	...
	function straightFeed(x, y, z, speed) 
	{
		if (typeof(x) == "object")
		{
...
*/	
	...
	function basicParamCheck(param, span)
	{
		param.__span = span;
		...
		if (param.outlineOnly)
			param.stepSize = (span - 2 * param.fineEdge) / 2; // outline only: only cut outermost trace -> nSteps = 1
		...
		if (param.nSteps == undefined)
			param.nSteps = Math.ceil((span - 2 * param.fineEdge) / param.stepSize) * 0.5;
	...
	this.rect = function(param) 
	{
		...
		for (var z = param.z0 - param.stepSizeZ; z >= param.z0 - param.depth; z -= param.stepSizeZ)
		{
			...
			for (var s = param.outlineOnly ? Math.ceil(param.nSteps) - 1 : 0; s < Math.ceil(param.nSteps); s++)
			{
				...
				var xdiff = (!portrait ? spine / 2 : 0) + (s + param.nSteps % 1) * param.stepSize;
				var ydiff = ( portrait ? spine / 2 : 0) + (s + param.nSteps % 1) * param.stepSize;
	...
	this.drill = function(param)
	{
		...
		basicParamCheck(param, param.radius);
		...
		for (var s = param.outlineOnly ? param.nSteps - 1 : 1; s < param.nSteps + (param.fineEdge ? 1 : 0); s++)
		{
			// calculate speeds & limits
			var r = s < param.nSteps ? (param.outlineOnly ? param.radius - param.fineEdge : param.stepSize * s) : param.radius;
			...
			if (speedZ > maxVSpeed)
			{
				var t2 = param.stepSizeZ / maxVSpeed;
				speedH = param.stepSize / t2;
			}
			
			// caluclate resulting feed needed
			var speedXYZ = speedH; // unneeded?
			
			lastPoint = { x: param.x - r, y: param.y };
			feed(lastPoint);
			this.dipIn(param.z0, param);
			command({ type: param.clockwise ? G2 : G3, i: r, j: 0, z: param.z0 - param.depth, f: speedXYZ, p: param.nStepsZ });
			command({ type: param.clockwise ? G2 : G3, i: r, j: 0, z: param.z0 - param.depth, f: speedH,   p: 1 });
			feed({ z: param.z0 });
		}
	...
	function findBestPath()
	{
		...
		for (var i = 0; i < that.options.nPathTrials; i++)
		{
			...
			var lastPart = rnd( remainder.length );
			for (var j = 0; j < parts.length; j++)
				if (j != lastPart)
					remainder.push(j);
