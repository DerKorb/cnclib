function cnclib(userOptions)
{
	...
	this.options = { z_safe: 2, nPathTrials: 10000, drillClockwise: true, maxVSpeed: 100, maxHSpeed: 400 };
	var parts = [];
	var that = this;
...
*/	
	...
	this.rect = function(param) 
	{
		...
		var fineEdge = (this.options.fineEdge && param.fineEdge == undefined) || param.fineEdge;
		...
		if (!param.stepSize && !param.stepFactor && !param.nSteps)
			param.stepFactor = this.options.stepFactor;
		...
		if (!param.stepSizeZ && !param.stepFactorZ && !param.nStepsZ)
			param.stepFactorZ = this.options.stepFactorZ;
		...
		for (var z = param.z0 - stepSizeZ; z >= param.z0 - param.depth; z -= stepSizeZ)
		{
			...
			for (var s = param.outlineOnly ? Math.ceil(param.nSteps) - 1 : 0; s < Math.ceil(param.nSteps); s++)
			{
				...
				var xdiff = (!portrait ? spine / 2 : 0) + (s + param.nSteps % 1) * stepSize;
				var ydiff = ( portrait ? spine / 2 : 0) + (s + param.nSteps % 1) * stepSize;
	...
	this.drill = function(param)
	{
		...
		if (!param.zsteps)
			param.zsteps = Math.ceil(param.d/(mill/2));
		
		if (!param.steps)
			param.steps = param.r/(mill/3);
			
		var speedH = this.options.maxVSpeed / param.d * param.r * 2 * Math.PI * param.zsteps;
		if (speedH > this.options.maxHSpeed)
			speedH = this.options.maxHSpeed;

		var p = {x: param.x, y: param.y};
		this.startPart(p);

		feed(p);
		
		feed({z: param.z});
		for (step = param.outlineOnly ? param.steps-1 : 1; step < param.steps; step++)
		{	
			r = (param.r-mill/2)*step/(param.steps-1);
			console.log(r, param.r, mill/2, step, param.steps);
			feed({x: param.x-r, y: param.y});
			command({type: param.clockwise ? G3 : G2, i: r, j: 0, z: param.z-param.d, p: param.zsteps, f: speedH});
			command({type: param.clockwise ? G3 : G2, i: r, j: 0, z: param.z-param.d, p: param.zsteps, f: speedH});
			feed({z: param.z});
		}
		
		return this.endPart({x: param.x-r, y: param.y});
	}	
...
>>>>>>> 543182fe7bee4d629707aa923fac31bcf10ee0b3
				render();
				break;
			case SIDE:
				measures("y", "z", "x");
