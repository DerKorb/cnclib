...
*/	
	...
	drill = function(param)
	{
		...
		var horizontalSpeed = maxVerticalSpeed / param.d * param.r*2*Math.PI;
		...
		{	
			...
			command({type: param.clockwise ? G3 : G2, i: r, j: 0, z: param.z-param.d, p: param.zsteps, f: horizontalSpeed});
			command({type: param.clockwise ? G3 : G2, i: r, j: 0, z: param.z-param.d, p: param.zsteps, f: horizontalSpeed});
			feed({z: param.z});
		}
		
		return endPart({x: param.x-r, y: param.y});
	}	
	...
	var bestPartOrder = false;
	var distMap = {};
		
	findBestPath = function(n)
	{
		if (!n)
			n = 100000;
	...
	function init() 
	{
		...
		button = $("<button>").appendTo(div).html("copy gcode").click(toText);
