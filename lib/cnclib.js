...
function cnclib(userOptions)
{
	...
	{
		if (f.toString().toLowerCase().match(/[xy]+/ig).indexOf("x") != -1 && f.toString().toLowerCase().match(/[xy]+/ig).indexOf("y") != -1)
		{
			f(-1,-1);
			f(-1,1);
			f(1,1);
	...
	function basicParamCheck(param, type)
	{
		...
		if (param.nSteps == undefined)
			param.nSteps = Math.ceil((span - 2 * param.fineEdge) / param.stepSize) * (type == "drill" ? 0.5 : 0.5);
	...
	this.drill = function(param)
	{
		...
		for (var s = param.outlineOnly ? param.nSteps - 1 : 0; s < param.nSteps + (param.fineEdge ? 0 : 1); s++)
		{
...
/*		if (first)
		...
		that.outlines.forEach( function (obj) 
		{
			if (obj.type == "rect")
			{
				...
				for (var zoff = 0; zoff < obj.param.depth + obj.param.stepSizeZ / 2; zoff += obj.param.stepSizeZ * 2)
				{
					...
					arcShape.moveTo( obj.param.x + (obj.param.width + c * mill) / 2, obj.param.y - (obj.param.height + 1 * mill) / 2 ); // ur

					arcShape.moveTo( obj.param.x - (obj.param.width + c * mill) / 2, obj.param.y - (obj.param.height + 1 * mill) / 2 ); // ul
					if (obj.param.extendCorners)
						drawArcS(arcShape, obj.param.x - (obj.param.width + c2 * mill) / 2, obj.param.y - (obj.param.height + c2 * mill) / 2, mill / 2,  7 / 4 * Math.PI,   3 / 4 * Math.PI); // ul
					
					arcShape.moveTo( obj.param.x - (obj.param.width + 1 * mill) / 2, obj.param.y + (obj.param.height + c * mill) / 2 ); // ol
					if (obj.param.extendCorners)
						drawArcS(arcShape, obj.param.x - (obj.param.width + c2 * mill) / 2, obj.param.y + (obj.param.height + c2 * mill) / 2, mill / 2,  5 / 4 * Math.PI,   1 / 4 * Math.PI); // ol

					arcShape.moveTo( obj.param.x + (obj.param.width + c * mill) / 2, obj.param.y + (obj.param.height + 1 * mill) / 2 ); // or
					if (obj.param.extendCorners)
						drawArcS(arcShape, obj.param.x + (obj.param.width + c2 * mill) / 2, obj.param.y + (obj.param.height + c2 * mill) / 2, mill / 2, 11 / 4 * Math.PI,   7 / 4 * Math.PI); // or
						
					if (obj.param.extendCorners)
					{
						arcShape.moveTo( obj.param.x + (obj.param.width + 1 * mill) / 2, obj.param.y - (obj.param.height + c * mill) / 2 ); // ur
						drawArcS(arcShape, obj.param.x + (obj.param.width + c2 * mill) / 2, obj.param.y - (obj.param.height + c2 * mill) / 2, mill / 2,  9 / 4 * Math.PI,   5 / 4 * Math.PI); // ur
					}
					...
					if (obj.param.extendCorners)
					{
						...
						line.vertices.push(new THREE.Vector3(obj.param.x - (obj.param.width + c * mill) / 2, obj.param.y - (obj.param.height + 1 * mill) / 2, z ));
						line.vertices.push(new THREE.Vector3(obj.param.x - (obj.param.width + 1 * mill) / 2, obj.param.y - (obj.param.height + 1 * mill) / 2, z ));
						line.vertices.push(new THREE.Vector3(obj.param.x - (obj.param.width + 1 * mill) / 2, obj.param.y - (obj.param.height + c * mill) / 2, z ));
			...
			if (obj.type == "drill")
			{
				...
				{
					var z = obj.param.z0 - zoff;
					drawArc(obj.param.x, obj.param.y, z, obj.param.radius + mill / 2, 0, 2 * Math.PI, true);
				}
				return;
			}			
		});
		...
		arrow(Y, 0x00FFFF);
		arrow(Z, 0xFFFF00);
		cube = new THREE.Mesh(new THREE.CubeGeometry(900,130,8), currentStyle.partMaterial);
		//scene.add(cube);
		cube.position.z = -4;
	}
