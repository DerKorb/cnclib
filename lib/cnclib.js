...
*/	
	...
	drill = function(param)
	{
		...
		feed({z: param.z});
		for (step = 1; step < param.steps; step++)
		{	
			r = (param.r-mill/2)*step/param.steps;
			feed({x: param.x-r, y: param.y});
			command({type: param.clockwise ? G2 : G3, i: r, j: 0, z: param.z-param.d, p: param.zsteps, x: param.x, y: param.y});
			command({type: param.clockwise ? G2 : G3, i: r, j: 0, z: param.z-param.d, p: 1, x: param.x, y: param.y, d: 0});
			feed({z: param.z});
		}
