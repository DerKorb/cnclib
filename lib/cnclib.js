...
/*		if (first)
		...
		that.outlines.forEach( function (obj) 
		{
			if (obj.type == "rect")
			{
				...
				mesh = new THREE.Mesh(new THREE.CubeGeometry(obj.param.width+2*mill, obj.param.height+2*mill, obj.param.depth), currentStyle.partMaterial);
				if (obj.param.x)
					mesh.position.x = obj.param.x;
				if (obj.param.y)
					mesh.position.y = obj.param.y;
				if (obj.param.z)
					mesh.position.z = obj.param.z-obj.param.depth/2;
				else
					mesh.position.z = -obj.param.depth/2;
				...
				for (var zoff = 0; zoff < obj.param.depth + obj.param.stepSizeZ / 2; zoff += obj.param.stepSizeZ * 2)
				{
					...
					line.vertices.push(new THREE.Vector3(obj.param.x + (obj.param.width + c * mill) / 2, obj.param.y - (obj.param.height + 1 * mill) / 2, z ));
					scene.add(new THREE.Line( line, currentStyle.outline));
					line = new THREE.Geometry();
					...
					if (obj.param.extendCorners)
					{
						...
						scene.add(new THREE.Line( line, currentStyle.outline2));
						...
						c = 1 - 1 / Math.sqrt(2);
						drawArc(obj.param.x - (obj.param.width + c * mill) / 2, obj.param.y + (obj.param.height + c * mill) / 2, z, mill / 2,  5 / 4 * Math.PI,   1 / 4 * Math.PI);
						drawArc(obj.param.x + (obj.param.width + c * mill) / 2, obj.param.y + (obj.param.height + c * mill) / 2, z, mill / 2, 11 / 4 * Math.PI,   7 / 4 * Math.PI);
						drawArc(obj.param.x + (obj.param.width + c * mill) / 2, obj.param.y - (obj.param.height + c * mill) / 2, z, mill / 2,  9 / 4 * Math.PI,   5 / 4 * Math.PI);
						drawArc(obj.param.x - (obj.param.width + c * mill) / 2, obj.param.y - (obj.param.height + c * mill) / 2, z, mill / 2,  7 / 4 * Math.PI,   3 / 4 * Math.PI);
					}
				}
				return;
			}
...
//		window.addEventListener( 'resize', onWindowResize, false );
	...
	function animate() 
	{
		...
		if (lastRender)
			waitTime = 1000 / fps - d.getMilliseconds() + lastRender.getMilliseconds();
