function cnclib(userOptions)
{
	...
	this.options = { z_safe: 2, nPathTrials: 10000, drillClockwise: true };
	var parts = [];
	var that = this;
...
*/	
	...
	this.drill = function(param)
	{
		...
		{	
			...
			command({type: param.clockwise ? G2 : G3, i: r, j: 0, z: param.z-param.d, p: param.zsteps});
			command({type: param.clockwise ? G2 : G3, i: r, j: 0, z: param.z-param.d, p: param.zsteps});
			feed({z: param.z});
		}
			...
			switch (command.type) 
			{
				...
				case G3:
					...
					for (n = 0; n <= 1.0001; n+=0.002)
					{
						...
						update({x: cx-Math.cos(Math.PI-n*command.p*2*Math.PI)*command.i, z: startz+(command.z-startz)*n, y: cy+Math.sin(Math.PI-n*command.p*2*Math.PI)*command.i });
					}
					scene.add(new THREE.Line( line, new THREE.LineDashedMaterial( currentStyle.G1 )));
				default:
					break;
