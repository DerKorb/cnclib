...
/*		params = ["G1", "X"+sanitize(x), "Y"+sanitize(y), "Z"+sanitize(z), "F"+sanitize(speed)];
	...
	rect = function(param) 
	{
		...
		if (param.x1 != undefined && param.y1 != undefined)
		{
			...
			assert(param.width != undefined && param.height != undefined, "Missing parameters (width/height or x2/y2)");
		...
		assert(param.width > mill && param.height > mill, "Rectangle too small");
		...
		if (param.width < fineEdge * 2 || param.height < fineEdge * 2)
			fineEdge = 0;
		
		var spine = param.width - param.height;
		var span = spine > 0 ? param.height : param.width;
		...
		{
			assert(param.stepFactor > 0 && param.stepFactor <= 1, "stepFactor needs to lie inbetween 0 and 1 (including)");
			param.stepSize = mill * param.stepFactor;
		}
		
		if (param.nSteps != undefined)
			assert(span / param.nSteps <= mill, "Not enough steps in rectangle");
		
		if (param.nSteps == undefined)
			param.nSteps = Math.ceil((span - 2 * fineEdge) / param.stepSize) * 0.5;
			
		assert(param.nSteps != 0, "Invalid number of steps");
			
		var stepSize = (span - 2 * fineEdge) / Math.floor(2 * param.nSteps);
		
		// z Parameters
		assert(param.depth > 0, "objects need to have a positive depth");
		
		if (param.z0 == undefined)
			param.z0 = 0;
			
		assert(param.z0 < 0, "objects need to be below the surface of your material");
		...
		{
			assert(param.stepFactorZ > 0, "stepFactorZ cannot be negative");
		...
		assert(param.nStepsZ != 0, "Invalid number of steps in z direction");
			
		var stepSizeZ = param.depth / param.nStepsZ;
		var firstCut = true;
		
		for (var z = z0; z >= z0 - param.depth; z -= stepSizeZ)
		{
			// dip into the material		
			if (param.nSteps % 1 == 0.5)
			{
				// cut the spine (from right to left)
				var p1 = { 	x: param.x + (spine > 0 ? spine / 2 : 0), 
							y: param.y + (spine < 0 ? spine / 2 : 0) };
				...
				dipIn(z, param);
				straighFeed( { 	x: param.x - (spine > 0 ? spine / 2 : 0), 
								y: param.y - (spine < 0 ? spine / 2 : 0) });
			}
			
			else for (var s = 0; s < Math.ceil(param.nSteps); s++)
			{
				// cut rect clockwise starting at the top left corner
				var xdiff = (spine > 0 ? spine / 2 : 0) + param.stepSize;
				var ydiff = (spine < 0 ? spine / 2 : 0) + param.stepSize;
				...
				dipIn(z, param);
				straightFeed( { x: param.x + xdiff, y: param.y - ydiff, speed: param.speed } );
				straightFeed( { x: param.x + xdiff, y: param.y + ydiff } );
				straightFeed( { x: param.x - xdiff, y: param.y + ydiff } );
				straightFeed( { x: param.x - xdiff, y: param.y - ydiff } );
			}
			...
			if (fineEdge)
			{
				...
				straightFeed( { x: param.x - xdiff, y: param.y - ydiff, speed: param.fineSpeed } );
				straightFeed( { x: param.x + xdiff, y: param.y - ydiff } );
				straightFeed( { x: param.x + xdiff, y: param.y + ydiff } );
				straightFeed( { x: param.x - xdiff, y: param.y + ydiff } );
				straightFeed( { x: param.x - xdiff, y: param.y - ydiff } );
			...
			switch (command.type) 
			{
				...
				case G3:
					...
					for (n = 0; n <= 1.0001; n+=0.02)
					{
						...
						update({x: command.x+Math.cos(n*command.p*2*Math.PI)*command.i, z: startz+(command.z-startz)*n, y: command.y+Math.sin(n*command.p*2*Math.PI)*command.i });
					}
					scene.add(new THREE.Line( line, new THREE.LineDashedMaterial( currentStyle.G1 )));
				default:
					break;
